...
listPhonemes    = {'n','d','t','r','ng','g','k','w','z','s','zh','z'};
...
for k=1:numPhrases
    ...
    for count_phoneme = 1:size(listPhonemes,2)
        avPhoneme_tot{count_phoneme,2} = extract_Phoneme_EPG_2018_11_28(EPG_parameters,listPhonemes{k},avPhoneme_tot{count_phoneme,2});
    end
end
...
for k = 1:numPhonemes
    sumActiv =sum([sum(sum(avPhoneme_tot{k,2}(:,1:120,:))) sum(sum(avPhoneme_tot{k,2}(:,121:240,:)))],3)/sum(avPhoneme_tot{k,2}(:));

    figure(1)
    imagesc(sum(avPhoneme_tot{k,2},3));colorbar
    colormap(jet2)
  %  title(strcat('msak: [',32,avPhoneme_tot{k,1},32,'], Asym:',num2str(sumActiv(1)),'/',num2str(sumActiv(2))),'fontsize',15)
    title(strcat('fsew: [',32,avPhoneme_tot{k,1},32,'], Asym:',num2str(sumActiv(1)),'/',num2str(sumActiv(2))),'fontsize',15)
    axis off
    
  %  filename=strcat('msak_sum_',avPhoneme_tot{k,1});
    filename=strcat('fsew_sum_',avPhoneme_tot{k,1});
    set(gcf,'color','w')
  ...
  %  filename=strcat('msak_montage_',avPhoneme_tot{k,1});
    filename=strcat('fsew_montage_',avPhoneme_tot{k,1});
    set(gcf,'color','w')
    set(gcf,'PaperPositionMode','auto')
    set(gcf,'InvertHardcopy','off')
    print('-djpeg','-r100',filename)
